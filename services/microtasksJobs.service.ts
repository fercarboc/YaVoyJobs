...
export interface MicrotaskJobPayload {
  title: string;
  description: string;
  category?: string;
  microtask_id: string;
...
export async function createMicrotaskJob(payload: MicrotaskJobPayload) {
  const { data, error } = await supabase.from("VoyJobs").insert([payload]).select().single();
